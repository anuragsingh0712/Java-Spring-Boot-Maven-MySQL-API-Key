...
import com.example.app.security.ApiKeyService;
...
@Tag(
    ...
    description = "Admin-gated endpoints to create, list, and revoke API keys")
public class ApiKeyController {
  ...
  private void verifyAdmin(String adminKey) {
    if (adminKey == null
        || configuredAdminKey == null
        || !MessageDigest.isEqual(
            adminKey.getBytes(StandardCharsets.UTF_8),
            configuredAdminKey.getBytes(StandardCharsets.UTF_8))) {
      throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "Invalid or missing admin key");
    }
  }
}
